  ...
  container.innerHTML = `
    <div class="page">
      <div class="section-header">
        <span class="section-title">📊 আমার ড্যাশবোর্ড</span>
      </div>
  ...
  try {
    ...
    el.innerHTML = `
      <div class="stats-row">
        ...
        <div class="stat-box"><div class="stat-num">${borrows.size}</div><div class="stat-lbl">ধার নিয়েছি</div></div>
  ...
  try {
    ...
    el.innerHTML = snap.docs.map(d=>{
      ...
      return `<div class="history-item type-order">
        ...
        <div class="history-date">📅 ${formatDate(o.createdAt)} · ${o.qty||1} কপি ${o.total?`· ৳${o.total}`:''}</div>
        ${o.type==='print'?`<div class="text-sm text-muted">ধরন: ${o.bindingType||''}</div>`:''}
      </div>`;
    }).join('');
...
async function loadMyBorrows() {
  ...
  try {
    ...
    el.innerHTML = snap.docs.map(d=>{
      ...
      const label=b.status==='returned'?'ফেরত':b.status==='approved'?'অনুমোদিত':b.status==='rejected'?'প্রত্যাখ্যাত':'অপেক্ষামান';
      return `<div class="history-item type-borrow">
        ...
        <div class="history-date">👤 মালিক: ${b.ownerName}</div>
        <div class="history-date">📅 ${b.fromDate} → ${b.toDate}</div>
        ${b.status==='approved'?`<a href="${buildWhatsAppLink(b.ownerPhone,`আমি "${b.bookTitle}" বইটি ফেরত দিতে চাই।`)}" target="_blank" class="btn-secondary btn-sm" style="text-decoration:none;display:inline-block;margin-top:6px;">📱 ফেরতের যোগাযোগ</a>`:''}
      </div>`;
    }).join('');
  } catch(e){ el.innerHTML=`<div class="empty-state"><p>লোড করতে সমস্যা</p></div>`; }
}
...
async function loadMyPersonalBooks() {
  ...
  try {
    ...
    if (snap.empty) { el.innerHTML=`<div class="empty-state"><div class="empty-icon">🏡</div><p>আপনার কোনো বই নেই</p><button class="btn-primary btn-sm" onclick="navigate('personal')" style="margin-top:12px;">বই যোগ করুন</button></div>`; return; }
    el.innerHTML = snap.docs.map(d=>{
      const b=d.data();
      return `<div class="history-item">
        ...
        <div class="history-date">📅 যোগ: ${formatDate(b.createdAt)}</div>
      </div>`;
    }).join('');
  } catch(e){ el.innerHTML=`<div class="empty-state"><p>লোড করতে সমস্যা</p></div>`; }
}
...
async function loadMyEbooks() {
  ...
  try {
    ...
    if (snap.empty) { el.innerHTML=`<div class="empty-state"><div class="empty-icon">📖</div><p>কোনো আপলোড নেই</p><button class="btn-primary btn-sm" onclick="navigate('ebook')" style="margin-top:12px;">বই আপলোড</button></div>`; return; }
    el.innerHTML = snap.docs.map(d=>{
      const b=d.data();
      return `<div class="history-item">
        ...
        <div style="display:flex;gap:6px;margin-top:6px;">
          <a href="${b.driveLink}" target="_blank" class="btn-secondary btn-sm" style="text-decoration:none;display:inline-block;">🔗 লিংক</a>
          <button class="btn-danger btn-sm" onclick="deleteMyEbook('${d.id}')">🗑️</button>
        </div>
      </div>`;
    }).join('');
  } catch(e){ el.innerHTML=`<div class="empty-state"><p>লোড করতে সমস্যা</p></div>`; }
...
async function deleteMyEbook(id) {
  if (!confirm('এই ই-বুক মুছে ফেলবেন?')) return;
  ...
  container.innerHTML = `
    <div class="page">
      <div class="card" style="max-width:400px;margin:40px auto;">
        <div style="background:var(--primary-dark);color:#fff;border-radius:var(--radius) var(--radius) 0 0;margin:-16px -16px 16px;padding:20px;text-align:center;font-family:var(--font-serif);font-size:18px;font-weight:700;">
          🔐 অ্যাডমিন লগইন
        </div>
  ...
  container.innerHTML = `
    <div class="page">
      ...
      <div style="display:flex;gap:8px;margin-bottom:14px;overflow-x:auto;padding-bottom:4px;">
        ...
        <button class="btn-secondary btn-sm" onclick="loadAdminTab('booklist')" id="atab-booklist">🛒 তালিকা</button>
      </div>
...
function loadAdminTab(tab) {
  ...
  switch(tab) {
    ...
    case 'booklist': loadAdminBooklist(); break;
  }
}
  ...
  try {
    ...
    el.innerHTML = snap.docs.map(d => {
      ...
      return `<div class="history-item type-borrow">
        ...
        <div class="history-date">🟢 মালিক: ${b.ownerName} (${b.ownerPhone})</div>
        <div class="history-date">📅 ${b.fromDate} → ${b.toDate}</div>
      </div>`;
    }).join('');
  } catch(e) { el.innerHTML=`<div class="empty-state"><p>লোড করতে সমস্যা</p></div>`; }
